...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # Get the knowledge base and tell it where we are
        kb = PropKB()
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())

        # Count how many actions (regardless of preconditions) separate current state from the goal
        for clause in self.goal:
            if clause not in kb.clauses:
                count += 1
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [expr('At(C2, SFO)'), expr('At(C2, ATL)'),
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), expr('At(C1, ATL)'), expr('At(C1, ORD)'),
           ...
           expr('At(C4, SFO)'), expr('At(C4, JFK)'), expr('At(C1, ATL)'),
